...
//    char                        buf[256];
    ...
    while ((usbDevice = IOIteratorNext(iterator)))
    {
        ...
        {
            XOkeyLog([NSString stringWithFormat:@"Unable to create a plug-in (%08x)", kr]);
            continue;
        }
        ...
        if (result || !dev)
        {
            XOkeyLog([NSString stringWithFormat:@"Couldn’t create a device interface (%08x)",
                   (int) result]);
            continue;
        }
        ...
        if ((vendor != kOurVendorID) || (product != kOurProductID) )//||
            //(release != 1))
        {
            XOkeyLog([NSString stringWithFormat:@"Found unwanted device (vendor = %d, product = %d)",
                   vendor, product]);
            (void) (*dev)->Release(dev);
        }else{
            XOkeyLog([NSString stringWithFormat:@"Found XOkey device! (vendor = %d, product = %d)",vendor,product]);
...
void RawDeviceRemoved(void *refCon, io_iterator_t iterator){
    ...
    while ((object = IOIteratorNext(iterator)))
    {
        ...
        if (kr != kIOReturnSuccess)
        {
            XOkeyLog([NSString stringWithFormat:@"Couldn’t release raw device object: %08x\n", kr]);
            continue;
        }else{
            XOkeyLog(@"Released XOkey object.");
            [[NSNotificationCenter defaultCenter]postNotificationName:XOKEY_UNPLUG object:nil];
        }
    }
}
...
IOReturn ConfigureDevice(IOUSBDeviceInterface **dev){
    ...
    if (kr)
    {
        XOkeyLog([NSString stringWithFormat:@"Couldn’t get configuration descriptor for index %d (err =%08x)\n", 0, kr]);
        return -1;
    }
    ...
    if (kr)
    {
        XOkeyLog([NSString stringWithFormat:@"Couldn’t set configuration to value %d (err = %08x)\n", 0, kr]);
        return -1;
    }
...
IOReturn FindInterfaces(IOUSBDeviceInterface **device)
{
    ...
    while ((usbInterface = IOIteratorNext(iterator)))
    {
        ...
        {
            XOkeyLog([NSString stringWithFormat:@"Unable to create a plug-in (%08x)\n", kr]);
            break;
        }
        ...
        if (result || !interface)
        {
            XOkeyLog([NSString stringWithFormat:@"Couldn’t create a device interface for the interface(%08x)\n", (int) result]);
            break;
        }
        ...
        XOkeyLog([NSString stringWithFormat:@"Interface class %d, subclass %d\n", interfaceClass,interfaceSubClass]);
        ...
        if (kr != kIOReturnSuccess)
        {
            XOkeyLog([NSString stringWithFormat:@"Unable to open interface (%08x)\n", kr]);
       ...
       if (kr != kIOReturnSuccess)
       {
           XOkeyLog([NSString stringWithFormat:@"Unable to get number of endpoints (%08x)\n", kr]);
       ...
       for (pipeRef = 1; pipeRef <= interfaceNumEndpoints; pipeRef++)
       {
           ...
           if (kr2 != kIOReturnSuccess)
               XOkeyLog([NSString stringWithFormat:@"Unable to get properties of pipe %d (%08x)\n",
                      pipeRef, kr2]);
           else
           {
               XOkeyLog([NSString stringWithFormat:@"PipeRef %d: ", pipeRef]);
               switch (direction)
               {
                   case kUSBOut:
                       message = "out";
               ...
               XOkeyLog([NSString stringWithFormat:@"direction %s, ", message]);
               ...
               XOkeyLog([NSString stringWithFormat:@"transfer type %s, maxPacketSize %d\n", message,
                      maxPacketSize]);
           }
       }
       ...
       */
       ...
       if (kr != kIOReturnSuccess)
       {
           XOkeyLog([NSString stringWithFormat:@"Unable to perform bulk read (%08x)\n", kr]);
           (void) (*interface)->USBInterfaceClose(interface);
           (void) (*interface)->Release(interface);
           break;
       }
       ...
       XOkeyLog([NSString stringWithFormat:@"Read \"%s\" (%u bytes) from bulk endpoint\n", gBuffer, (unsigned int)numBytesRead]);
       ...
       if (kr != kIOReturnSuccess)
           ...
           XOkeyLog([NSString stringWithFormat:@"Unable to create asynchronous event source(%08x)\n", kr]);
           (void) (*interface)->USBInterfaceClose(interface);
           (void) (*interface)->Release(interface);
           break;
        }
      ...
      XOkeyLog(@"Asynchronous event source added to run loop\n");
...
void WriteCompletion(void *refCon, IOReturn result, void *arg0)
{
    ...
    XOkeyLog(@"Asynchronous write complete\n");
    if (result != kIOReturnSuccess)
    {
        XOkeyLog([NSString stringWithFormat:@"error from asynchronous bulk write (%08x)\n", result]);
        (void) (*interface)->USBInterfaceClose(interface);
        (void) (*interface)->Release(interface);
        return;
    }
    XOkeyLog([NSString stringWithFormat:@"Wrote \"%s\" (%u bytes) to bulk endpoint\n", kTestMessage,
    ...
    if (result != kIOReturnSuccess)
    {
        XOkeyLog([NSString stringWithFormat:@"Unable to perform asynchronous bulk read (%08x)\n", result]);
        (void) (*interface)->USBInterfaceClose(interface);
        (void) (*interface)->Release(interface);
        return;
    }
...
void ReadCompletion(void *refCon, IOReturn result, void *arg0)
{
    ...
    XOkeyLog([NSString stringWithFormat:@"Asynchronous bulk read complete\n"]);
    if (result != kIOReturnSuccess) {
        XOkeyLog([NSString stringWithFormat:@"error from async bulk read (%08x)\n", result]);
        (void) (*interface)->USBInterfaceClose(interface);
        (void) (*interface)->Release(interface);
        return;
    }
...
- (int)enumerateUSB{
    ...
    if (kr || !masterPort)
    {
        XOkeyLog([NSString stringWithFormat:@"ERR: Couldn’t create a master I/O Kit port(%08x)\n", kr]);
        return -1;
    }
    ...
    if (!matchingDict)
    {
        XOkeyLog([NSString stringWithFormat:@"Couldn’t create a USB matching dictionary\n"]);
